...
if(!args['source-root']||!args['contract-root']||!args['factory-receipt']||!args['output-root']){
  throw new Error('Usage: write-owner-m6-full-rig-r2.mjs --source-root <assets/source> --contract-root <assets/contracts> --factory-receipt <receipt.json> --output-root <output>');
}
...
const result=buildOwnerM6FullRigPackageR2({
...
await Promise.all([
  writeFile(join(modelDirectory,'m6-owner-full-rig-r2.glb'),result.glb),
  writeFile(join(outputRoot,'m6-owner-full-rig-r2.visual.json'),result.manifestText,'utf8'),
  writeFile(join(outputRoot,'m6-owner-full-rig-r2.report.json'),`${JSON.stringify(result.report,null,2)}\n`,'utf8'),
]);
console.log(`Owner M6 full rig R2 written: ${result.glb.length} bytes · ${result.report.output.sha256} · ${result.report.output.realBindingCount} real bindings.`);
