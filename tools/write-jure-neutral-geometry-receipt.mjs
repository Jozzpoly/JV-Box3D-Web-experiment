...
function gitText(args, label, lowercase = true) {
...
function requireTrackedWorktreeClean() {
  try {
    ...
    throw new Error(
      "Refusing to export JV neutral geometry receipt: tracked worktree differs from HEAD.",
    );
  }
}
...
function requireExpectedOrigin() {
  ...
  const accepted = new Set([
    `https://github.com/${JV_WEB_REPOSITORY}`,
    `git@github.com:${JV_WEB_REPOSITORY}`,
    `ssh://git@github.com/${JV_WEB_REPOSITORY}`,
  ]);
  if (!accepted.has(origin)) {
    throw new Error(
      `Refusing to export JV neutral geometry receipt: origin ${JSON.stringify(origin)} does not identify ${JV_WEB_REPOSITORY}.`,
    );
  }
}
