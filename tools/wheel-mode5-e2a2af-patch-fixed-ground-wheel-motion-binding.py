...
# Start from the inverse relative tilt and retain the historical X/Y locks through settling.
# The locks are opened only for the bounded crossing window, preventing the unlocked wheel from
# accumulating contact-induced off-axis drift before the experiment starts.
...
new_motion = '''        if ( step == settleSteps )\n        {\n            b3MotionLocks freeTilt = b3Body_GetMotionLocks( wheelBody );\n            freeTilt.angularX = false;\n            freeTilt.angularY = false;\n            b3Body_SetMotionLocks( wheelBody, freeTilt );\n        }\n        if ( step == settleSteps + motionSteps )\n        {\n            b3MotionLocks lockedTilt = b3Body_GetMotionLocks( wheelBody );\n            lockedTilt.angularX = true;\n            lockedTilt.angularY = true;\n            b3Body_SetMotionLocks( wheelBody, lockedTilt );\n        }\n\n        float commandedAngle = initialAngle;\n        float commandedTiltRate = 0.0f;\n        if ( step >= settleSteps && step < settleSteps + motionSteps )\n        {\n            int motionIndex = step - settleSteps;\n            commandedAngle = initialAngle + commandedAngularX * dt * (float)motionIndex;\n            commandedTiltRate = commandedAngularX;\n        }\n        else if ( step >= settleSteps + motionSteps )\n        {\n            commandedAngle = initialAngle + commandedAngularX * dt * (float)motionSteps;\n        }\n\n        if ( step >= settleSteps && step < settleSteps + motionSteps )\n        {\n            // Command the world angular velocity that advances wheel-local ground-normal Z while\n            // preserving the 5 rad/s spin. Re-command every outer step to bound contact-torque drift.\n            b3Vec3 wheelAngularVelocity = e1Vec(\n                -commandedTiltRate,\n                spinRadiansPerSecond * sinf( commandedAngle ),\n                spinRadiansPerSecond * cosf( commandedAngle ) );\n            b3Body_SetAngularVelocity( wheelBody, wheelAngularVelocity );\n        }\n        else\n        {\n            b3Body_SetAngularVelocity( wheelBody, e1Vec( 0.0f, 0.0f, spinRadiansPerSecond ) );\n        }\n\n        b3World_Step( worldId, dt, subStepCount );\n'''
...
new_angle = '''        b3Quat wheelRotation = b3Body_GetRotation( wheelBody );\n        b3Vec3 groundUpWheelLocal = b3InvRotateVector( wheelRotation, b3Vec3_axisY );\n        float relativeTiltSin = b3ClampFloat( groundUpWheelLocal.z, -1.0f, 1.0f );\n        float angle = asinf( relativeTiltSin );\n        float axial = fabsf( relativeTiltSin );\n'''
