...
if (!resultPath || !['control', 'yaw'].includes(mode)) {
  throw new Error('usage: node wheel-mode5-rq2c-orientation-validate.mjs <result.json> <control|yaw>');
}

const result = JSON.parse(await readFile(resultPath, 'utf8'));
assert.equal(result.schemaVersion, 1, 'unsupported RQ2C result schema');
assert.equal(result.method, 'RQ2C_ROTATED_HEADING_ORIENTATION_EQUIVALENCE', 'RQ2C method drifted');
...
assert.equal(result.apparatus?.mountHertz, 120, 'RQ2C mount stiffness drifted');
assert.equal(result.apparatus?.mountDampingRatio, 1, 'RQ2C mount damping drifted');
assert.equal(result.apparatus?.translation, 'FULLY_FREE_NO_WORLD_AXIS_LOCKS', 'RQ2C translation scope drifted');
assert.equal(result.apparatus?.challengeDegrees, 3.5, 'RQ2C challenge angle drifted');
assert.equal(result.apparatus?.maxAxisErrorDegrees, 0.035, 'RQ2C axis budget drifted');
assert.equal(result.apparatus?.maxHeadingErrorDegrees, 0.035, 'RQ2C heading budget drifted');
...
if (mode === 'yaw' && failures.length === 0) {
  ...
  const mirrorDiagnostics = {
    ...
    crossHeadingSpeedDeltaMmPerS: Math.abs(plus.settledMaxAbsCrossHeadingSpeedMmPerS - minus.settledMaxAbsCrossHeadingSpeedMmPerS),
    crossTrackDeltaMm: Math.abs(plus.settledMaxAbsCrossTrackMm - minus.settledMaxAbsCrossTrackMm),
  };
  console.log('WHEEL_MODE5_RQ2C_MIRROR_DIAGNOSTICS', JSON.stringify(mirrorDiagnostics));
}

if (failures.length > 0) {
  const classification = mode === 'control'
    ? 'APPARATUS_INVALID_TRANSLATIONALLY_FREE_CONTROL'
    : 'RQ2C_ORIENTATION_GATE_FAIL';
  console.error('WHEEL_MODE5_RQ2C_CLASSIFICATION', classification);
  console.error('WHEEL_MODE5_RQ2C_GATE_FAILURES', JSON.stringify(failures));
  process.exitCode = 1;
} else {
  console.log('WHEEL_MODE5_RQ2C_CLASSIFICATION', mode === 'control' ? 'CONTROL_PASS' : 'YAW_PAIR_PASS');
  console.log('WHEEL_MODE5_RQ2C_GATES_OK');
}
