...
if (!outputPath || angleArgs.length === 0) {
  throw new Error('usage: node wheel-mode5-rq2c3-direct-guide-run.mjs <output.json> <yaw-deg> [yaw-deg ...]');
}
...
for (const yaw of requestedYawDegrees) {
  ...
  assert.ok([0, 3.5, -3.5].some((allowed) => Math.abs(yaw - allowed) <= 1e-9), `unsupported RQ2C3 yaw: ${yaw}`);
}
assert.equal(new Set(requestedYawDegrees).size, requestedYawDegrees.length, 'duplicate RQ2C3 yaw arguments');

const b3 = await Box3D();
assert.equal(typeof b3.rq2c3RunOuterP75DirectGuide, 'function', 'RQ2C3 direct-guide binding missing');
...
for (const yawDegrees of requestedYawDegrees) {
  const raw = b3.rq2c3RunOuterP75DirectGuide(yawDegrees);
  assert.equal(raw.valid, true, `RQ2C3 yaw ${yawDegrees}: invalid raw run ${JSON.stringify(raw)}`);
  assert.equal(raw.apparatus, 'RQ2C3_DIRECT_PARALLEL_LOCAL_Z_LINEAR_GUIDE', 'RQ2C3 apparatus identity drifted');
  assert.ok(Math.abs(raw.yawDegrees - yawDegrees) <= 1e-5, `RQ2C3 yaw echo drifted: ${raw.yawDegrees} vs ${yawDegrees}`);
  ...
  cases[String(yawDegrees)] = {
    ...
    settledMaxAbsPlaneSeparationMm: finite(raw.settledMaxAbsPlaneSeparation * 1000, 'settledMaxAbsPlaneSeparationMm'),
    ...
    finalPlaneSeparationMm: finite(raw.finalPlaneSeparation * 1000, 'finalPlaneSeparationMm'),
  };
}

const result = {
  schemaVersion: 1,
  method: 'RQ2C3_DIRECT_LOCAL_AXIS_GUIDE_EQUIVALENCE',
  role: 'Direct point-to-plane local axle-axis guide qualification with no helper bodies; bounded post-RH0 research apparatus only.',
  ...
  apparatus: {
    topology: 'static-reference -> one ParallelJoint -> wheel; direct scalar local-Z linear guide; no helper bodies',
    translation: 'LOCAL_AXLE_AXIS_ONLY_CONSTRAINED_HEADING_AND_VERTICAL_FREE',
    mountHertz: 120,
    mountDampingRatio: 1,
...
console.log('WHEEL_MODE5_RQ2C3_DIRECT_GUIDE_RESULT', JSON.stringify(result));
console.log('WHEEL_MODE5_RQ2C3_DIRECT_GUIDE_EXECUTED');
