// Wheel-mode5 RQ2C3 direct local-axis translational guide.
//
// This suite returns to the simple RQ2C0 static-reference + wheel topology.
// A transient, default-off ParallelJoint extension constrains only relative
// translation along frame A local +Z (the yaw-rotated axle axis). There are no
// helper bodies and no world-axis motion locks.

static val rq2c3RunOuterP75DirectGuide( float yawDegrees )
{
    val result = val::object();
    if ( b3IsValidFloat( yawDegrees ) == false || rq2cSupportedYawDegrees( yawDegrees ) == false )
    {
        result.set( "valid", false );
        result.set( "error", "RQ2C3 supports only 0 / +3.5 / -3.5 degree yaw" );
        return result;
    }
    ...
    const float linearGuideHertz = 240.0f;
    const float linearGuideDampingRatio = 2.0f;
    ...
    referenceBodyDef.position = initialCenter;
    ...
    b3Joint_SetConstraintTuning( mountJoint, linearGuideHertz, linearGuideDampingRatio );
    b3ParallelJoint_EnableLinearAxisGuide( mountJoint, true );
         ...
         b3Shape_IsValid( wheelShape ) == false )
    {
        ...
        result.set( "valid", false );
        result.set( "error", "RQ2C3 direct guide, ground or wheel shape invalid" );
        return result;
    }
    ...
    float settledMaxAbsPlaneSeparation = 0.0f;
    ...
    for ( int step = 0; step < stepCount; ++step )
    {
        ...
        float planeSeparation = rq2cDot( rq2cSub( p, initialCenter ), targetAxle );
        ...
        if ( step >= settleStep )
        {
            ...
            settledMaxAbsPlaneSeparation = b3MaxFloat( settledMaxAbsPlaneSeparation, fabsf( planeSeparation ) );
    ...
    float finalPlaneSeparation = rq2cDot( rq2cSub( finalPosition, initialCenter ), targetAxle );

    result.set( "valid", true );
    result.set( "apparatus", "RQ2C3_DIRECT_PARALLEL_LOCAL_Z_LINEAR_GUIDE" );
    ...
    result.set( "linearGuideHertz", linearGuideHertz );
    result.set( "linearGuideDampingRatio", linearGuideDampingRatio );
    ...
    result.set( "settledMaxAbsPlaneSeparation", settledMaxAbsPlaneSeparation );
    ...
    result.set( "finalPlaneSeparation", finalPlaneSeparation );
