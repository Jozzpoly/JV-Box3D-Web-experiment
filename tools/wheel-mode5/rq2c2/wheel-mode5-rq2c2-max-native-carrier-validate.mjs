...
if (!resultPath || !['control', 'yaw'].includes(mode)) {
  throw new Error('usage: node wheel-mode5-rq2c2-max-native-carrier-validate.mjs <result.json> <control|yaw>');
}

const result = JSON.parse(await readFile(resultPath, 'utf8'));
assert.equal(result.schemaVersion, 1, 'unsupported RQ2C2 result schema');
assert.equal(result.method, 'RQ2C2_MAX_NATIVE_GENERIC_CONSTRAINT_CARRIER', 'RQ2C2 method drifted');
...
assert.equal(result.dependencies?.rq2c1DiagnosticSource, '2c54b4960c58cb7ca56d5f1583227ad0ba637958', 'RQ2C1D source drifted');
assert.equal(result.dependencies?.rq2c1DiagnosticRun, 33966947600, 'RQ2C1D run drifted');
...
assert.equal(result.apparatus?.mountHertz, 120, 'angular mount hertz drifted');
assert.equal(result.apparatus?.mountDampingRatio, 1, 'angular mount damping drifted');
assert.equal(result.apparatus?.carrierConstraintHertz, 240, 'carrier constraint hertz drifted');
assert.equal(result.apparatus?.carrierConstraintDampingRatio, 2, 'carrier constraint damping drifted');
assert.equal(result.apparatus?.solverMaxEffectiveConstraintHertz, 240, 'solver max-effective hertz provenance drifted');
assert.equal(result.apparatus?.noHertzSweep, true, 'RQ2C2 must remain a single max-native falsifier');
assert.equal(result.apparatus?.challengeDegrees, 3.5, 'challenge angle drifted');
assert.equal(result.apparatus?.maxAxisErrorDegrees, 0.035, 'axis budget drifted');
assert.equal(result.apparatus?.maxHeadingErrorDegrees, 0.035, 'heading budget drifted');
...
for (const yaw of expectedYaws) {
  ...
  gate(Number.isFinite(c.wheelMassKg) && c.wheelMassKg > 0, `${yaw}: invalid wheel mass ${c.wheelMassKg}`);
  ...
  gate(Math.abs(c.guideBodyMassKg / c.wheelMassKg - 0.005) <= 1e-7, `${yaw}: measured guide/wheel mass ratio ${c.guideBodyMassKg / c.wheelMassKg}`);
  gate(c.mountHertz === 120 && c.mountDampingRatio === 1, `${yaw}: angular mount tuning drifted`);
  gate(c.carrierConstraintHertz === 240 && c.carrierConstraintDampingRatio === 2, `${yaw}: carrier constraint tuning drifted`);
  for (const [joint, tuning] of Object.entries(c.configuredJointTuning ?? {})) {
    gate(Array.isArray(tuning) && tuning[0] === 240 && tuning[1] === 2, `${yaw}: ${joint} configured tuning ${JSON.stringify(tuning)}`);
  }
  gate(Object.keys(c.configuredJointTuning ?? {}).length === 3, `${yaw}: expected three configured carrier joints`);
...
const diagnostics = Object.fromEntries(expectedYaws.map((yaw) => {
  ...
  return [String(yaw), {
    ...
    maxSledCrossMm: c.settledMaxSledCrossMm,
    maxCarrierCrossMm: c.settledMaxCarrierCrossMm,
    maxSlipMmPerS: c.settledMaxAbsSlipMmPerS,
    maxAxisErrorDegrees: c.settledMaxAxisErrorDegrees,
    maxHeadingErrorDegrees: c.settledMaxHeadingErrorDegrees,
  }];
}));
console.log('WHEEL_MODE5_RQ2C2_DIAGNOSTICS', JSON.stringify(diagnostics));

if (mode === 'yaw' && failures.length === 0) {
  const plus = result.cases['3.5'];
  const minus = result.cases['-3.5'];
  console.log('WHEEL_MODE5_RQ2C2_MIRROR_DIAGNOSTICS', JSON.stringify({
...
if (failures.length > 0) {
  console.error('WHEEL_MODE5_RQ2C2_CLASSIFICATION', mode === 'control' ? 'MAX_NATIVE_CHAIN_CONTROL_FAIL' : 'MAX_NATIVE_CHAIN_YAW_GATE_FAIL');
  console.error('WHEEL_MODE5_RQ2C2_GATE_FAILURES', JSON.stringify(failures));
  process.exitCode = 1;
} else {
  console.log('WHEEL_MODE5_RQ2C2_CLASSIFICATION', mode === 'control' ? 'MAX_NATIVE_CHAIN_CONTROL_PASS' : 'MAX_NATIVE_CHAIN_YAW_PAIR_PASS');
  console.log('WHEEL_MODE5_RQ2C2_GATES_OK');
}
