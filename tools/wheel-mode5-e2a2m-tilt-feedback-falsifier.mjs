...
function runUnlocked(spin) {
  const run = b3.e2a2RunFlatP75GroundCarrier(0, spin, true);
  validate(run, `unlocked spin=${spin}`);
  return compact(run);
}

function runLocked(spin) {
  const run = b3.e2a2mRunFlatP75GroundCarrierTiltLocked(0, spin, true);
  validate(run, `tilt-locked spin=${spin}`);
  return compact(run);
}

const unlocked0 = runUnlocked(0);
const unlocked40 = runUnlocked(40);
const locked0 = runLocked(0);
const locked40 = runLocked(40);

function comparison(spin0, spin40) {
  return {
...
const result = {
  scope: 'E2a2m causal tilt-feedback falsifier. Same P75 two-point real b3Wheel, friction=0, 2 s, 4 substeps. Compare free angular X/Y vs motionLocks angularX+angularY while axial Z roll remains free.',
  unlockedComparison: comparison(unlocked0, unlocked40),
  lockedComparison: comparison(locked0, locked40),
  unlocked0,
  unlocked40,
  locked0,
  locked40,
...
console.log(`E2A2M_TILT_FEEDBACK_RESULT ${JSON.stringify(result)}`);
console.log('E2A2M_TILT_FEEDBACK_EXECUTED');
