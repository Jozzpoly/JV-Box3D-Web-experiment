...
Write-Host "Precondition: run 'git fetch origin --prune' immediately before this script for current origin refs."
...
Write-Host "Reviewed remote refs:   $($publicData.metrics.reviewedRemoteBranches)"
Write-Host "Blocked orphan refs:    $($publicData.metrics.blockedOrphanBranches)"
Write-Host "Unknown remote refs:    $($publicData.metrics.unknownRemoteBranches)"
Write-Host "Unclassified tags:      $($publicData.metrics.unclassifiedTags)"
Write-Host "Ref-policy status:      $($publicData.publicRefPolicy.status)"
Write-Host "HEAD/origin candidate:  $($publicData.publicRefPolicy.headCommit -eq $publicData.publicRefPolicy.remoteCandidateCommit)"
