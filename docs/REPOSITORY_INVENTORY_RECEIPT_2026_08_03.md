...
for all three forensic workflows.

They are not product CI and must not be run again unless:

- an exact source commit changes;
- a receipt method changes materially;
- a missing evidence question requires the cost;
- the rerun is named and justified before execution.

## 8. Coverage vocabulary

The project uses these distinct claims:
...
The corrected public inventory establishes `BYTE_INVENTORIED` only. The 34 contaminated web files have also received pass-1 semantic classification. Selected native JV, wheel research, binding, host and asset paths have deeper review. The rest must not be described as fully understood yet.
...
- locate files that have not yet received semantic review;
