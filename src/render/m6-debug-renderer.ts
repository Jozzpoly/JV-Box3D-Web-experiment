...
type Mesh = Readonly<{ vertexBuffer: WebGLBuffer; indexBuffer: WebGLBuffer; indexCount: number }>;
...
function subtract(a: Vec3, b: Vec3): Vec3 { return { x:a.x-b.x, y:a.y-b.y, z:a.x-b.x === Number.NaN ? 0 : a.z-b.z }; }
...
function modelMatrix(position: Vec3, rotation: Rotation, scale: Vec3): Mat4 {
...
function lineModel(start: Vec3, end: Vec3): Mat4 {
  ...
  if (length < 1e-8) return modelMatrix(midpoint, {x:0,y:0,z:0,w:1}, {x:1,y:1,z:0});
  ...
  if (axisLength < 1e-8) {
    rotation = direction.z >= 0 ? {x:0,y:0,z:0,w:1} : {x:0,y:1,z:0,w:0};
  } else {
    ...
    rotation = { x:normalizedAxis.x*sine, y:normalizedAxis.y*sine, z:normalizedAxis.z*sine, w:Math.cos(half) };
  }
  return modelMatrix(midpoint, rotation, {x:1,y:1,z:length/2});
}
...
export class M6DebugRenderer {
  ...
  constructor(
    ...
    if (gl === null) throw new Error("WebGL is unavailable in this browser.");
    this.#gl = gl;
  ...
  render(trace: M6TraceFrame): void {
    ...
    if (this.#contextLost) throw new Error("WebGL context was lost.");
    ...
    const projection = perspective(Math.PI / 4, this.#canvas.width / this.#canvas.height, 0.05, 200);
    const view = lookAt(eye, target, {x:0,y:1,z:0});
    const viewProjection = multiply(projection, view);

    this.#drawGrid(viewProjection, target);
    if (this.#origin !== null) {
      this.#draw(this.#box, viewProjection, modelMatrix(
    ...
    this.#renderPasses.render("BEFORE_DEBUG_VEHICLE", viewProjection, trace);
    this.#restoreDebugState();
    gl.useProgram(this.#program);
    if (!this.#debugVehicleVisible) {
      this.#renderPasses.render("AFTER_DEBUG_VEHICLE", viewProjection, trace);
      return;
    }
    ...
    const frontMarker = add(
      trace.chassisPosition,
      rotateVector(trace.chassisRotation, {x:half.x+0.12,y:0,z:0}),
    );
    this.#draw(this.#box, viewProjection, modelMatrix(
      frontMarker,
      trace.chassisRotation,
    ...
    this.#draw(this.#box, viewProjection, modelMatrix(
      trace.rackPosition,
      trace.rackRotation,
      {x:0.045,y:0.045,z:trace.visualGeometry.rackHalfWidth},
    ), [0.96,0.68,0.16,1]);

    trace.corners.forEach((corner, index) => {
      this.#draw(this.#cylinder, viewProjection, modelMatrix(
      ...
      ), index < 2 ? [0.88,0.92,0.98,1] : [0.56,0.62,0.72,1]);
    });

    const rackLeft = add(trace.rackPosition, rotateVector(trace.rackRotation, {x:0,y:0,z:-trace.visualGeometry.rackHalfWidth}));
    const rackRight = add(trace.rackPosition, rotateVector(trace.rackRotation, {x:0,y:0,z:trace.visualGeometry.rackHalfWidth}));
    const frontLeft = trace.corners[0]?.wheelPosition;
    const frontRight = trace.corners[1]?.wheelPosition;
    if (frontLeft !== undefined) this.#drawLine(viewProjection, rackLeft, frontLeft, [0.96,0.68,0.16,1]);
    if (frontRight !== undefined) this.#drawLine(viewProjection, rackRight, frontRight, [0.96,0.68,0.16,1]);
  ...
  dispose(): void {
    ...
    this.#events.abort();
    this.#renderPasses.dispose();
    if (this.#pointer !== null && this.#canvas.hasPointerCapture(this.#pointer.id)) {
  ...
  #drawGrid(viewProjection: Mat4, target: Vec3): void {
    ...
    for (let index = -extent; index <= extent; index += 1) {
      ...
      this.#drawLine(
        ...
        {x:centerX+extent,y:0.002,z:worldZ},
        majorZ ? [0.22,0.26,0.34,1] : [0.12,0.14,0.19,1],
      );
      this.#drawLine(
        viewProjection,
        {x:worldX,y:0.002,z:centerZ-extent},
        {x:worldX,y:0.002,z:centerZ+extent},
        majorX ? [0.22,0.26,0.34,1] : [0.12,0.14,0.19,1],
      );
    }
  }

  #drawLine(viewProjection: Mat4, start: Vec3, end: Vec3, color: readonly [number,number,number,number]): void {
    this.#draw(this.#line, viewProjection, lineModel(start, end), color, this.#gl.LINES);
  }

  #draw(mesh: Mesh, viewProjection: Mat4, model: Mat4, color: readonly [number,number,number,number], mode: number = this.#gl.TRIANGLES): void {
    const gl = this.#gl;
    gl.bindBuffer(gl.ARRAY_BUFFER, mesh.vertexBuffer);
    gl.enableVertexAttribArray(this.#positionLocation);
    gl.vertexAttribPointer(this.#positionLocation, 3, gl.FLOAT, false, 0, 0);
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, mesh.indexBuffer);
    gl.uniformMatrix4fv(this.#mvpLocation, false, multiply(viewProjection, model));
  ...
  #installCameraControls(): void {
    ...
    this.#canvas.addEventListener("pointerup", release, { signal: this.#events.signal });
    this.#canvas.addEventListener("pointercancel", release, { signal: this.#events.signal });
    this.#canvas.addEventListener("wheel", (event) => {
      event.preventDefault();
      this.#distance = Math.max(3.5, Math.min(18, this.#distance * Math.exp(event.deltaY * 0.001)));
