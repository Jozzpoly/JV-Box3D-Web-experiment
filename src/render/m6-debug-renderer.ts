...
function createShader(
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    ...
    throw new Error(
      `WebGL shader compilation failed: ${message}`,
    );
  }
  return shader;
}
...
function lineModel(
  ...
  const axis = cross({ x: 0, y: 0, z: 1 }, direction);
...
export class M6DebugRenderer {
  ...
  render(trace: M6TraceFrame): void {
    ...
    this.#draw(
      ...
      modelMatrix(
        ...
        trace.visualGeometry.chassisHalfExtents,
      ),
      [0.22, 0.52, 0.92, 1],
    );
  ...
  displacement(trace: M6TraceFrame): number {
    if (
      this.#origin === null ||
    ...
    return Math.hypot(
      trace.chassisPosition.x - this.#origin.x,
      trace.chassisPosition.z - this.#origin.z,
    );
  }
