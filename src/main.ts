...
async function start(): Promise<void> {
  ...
  statusElement.textContent = `Box3D ${version.major}.${version.minor}.${version.revision} · rig M6/M7`;

  const render = createRenderContext(canvas);
  const worldDef = b3.b3DefaultWorldDef();
  worldDef.gravity = { x: 0, y: -9.81, z: 0 };
  worldDef.enableContinuous = false; // Same deliberate vehicle-lab decision as current JV.
  ...
  const frame = (milliseconds: number) => {
    ...
    if (telemetryClock >= 0.2) {
      ...
      telemetryElement.innerHTML = [
        ...
        `rack: ${telemetry.rackTravel.toFixed(3)} m`,
