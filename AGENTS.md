...
- public `main` is the steady-state public artifact and GitHub Pages authority; exact rollback/evidence commits live in normal Git history rather than permanent release branch families.
...
Owner acceptance is scoped. A working public build does not make provisional steering, rig geometry or handling final truth.
...
No agent-side coordinate guessing. No implicit identity transform. No public artifact change in the same first consumer slice.
...
- Code-only public releases may carry forward the already-published exact scan.
