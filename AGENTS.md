...
- `preview/owner-control` is a permanent operational Owner Preview control lane, not product source authority. `preview/owner.json` identifies the exact executable source commit currently deployed for Owner testing and may additionally pin explicitly approved static artifact layers.
...
`preview/owner-control` is a special permanent operational control lane and does not count as a competing product work lane. It may point only at exact committed executable source candidates and explicitly pinned approved static layers; it must not become a second source tree.
...
Owner Preview is an R&D/testing surface produced from an exact committed executable source candidate. It is not accepted release authority.

- `preview/owner.json` must identify one exact 40-character executable source SHA.
- If Preview composes approved static data that was not generated by that executable source, the pointer must pin the layer by exact repository/commit/provenance and the deployed artifact must record the composition separately from executable build identity.
- Preview build identity must match the exact executable source candidate and a clean committed tree.
- Pinned preserved data must be integrity-checked against its approved release receipt or equivalent exact provenance before deploy.
- Owner Preview should preserve accepted product capabilities unrelated to the active experiment. A deliberate capability omission must be explicit, scoped and treated as an open Preview gap rather than silently becoming the new normal.
