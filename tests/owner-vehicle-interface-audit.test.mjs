...
test('front upper audit keeps authored-vs-physical attachment evidence as measurement, not acceptance',async()=>{
  const audit=buildOwnerM6InterfaceAudit(await inputs());
  for(const corner of ['fl','fr']){
    const upper=audit.corners[corner].interfaces.upperHinge;
    assert.ok(
      upper.authoredToCurrentMeters>0,
      `${corner}: authored and physical upper attachment references must remain independently measurable`,
    );
    assert.ok(
