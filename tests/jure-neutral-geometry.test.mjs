...
const TEST_SOURCE = Object.freeze({
  ...
  producer: Object.freeze({
    repository: JV_WEB_REPOSITORY,
    commit: "0123456789abcdef0123456789abcdef01234567",
  }),
  configReceipt: Object.freeze({
    path: JV_M6_FACTORY_RECEIPT_PATH,
    gitBlob: "89abcdef0123456789abcdef0123456789abcdef",
...
test("neutral receipt exporter binds repository and producer identity only after fail-closed guards", () => {
  assert.equal(
    neutralExporterSource.includes('["diff", "--quiet", "HEAD", "--"]'),
    true,
  );
  assert.equal(
    neutralExporterSource.includes('["config", "--get", "remote.origin.url"]'),
    true,
  );
  const cleanGuardCall = neutralExporterSource.indexOf("requireTrackedWorktreeClean();");
  const originGuardCall = neutralExporterSource.indexOf("requireExpectedOrigin();");
  ...
  assert.ok(originGuardCall >= 0, "missing expected-origin guard");
  ...
  assert.ok(originGuardCall < producerRead, "producer identity is read before repository-origin guard");
});
...
test("JV neutral mechanism v1 exposes one explicit engine-neutral rig space and coherent FL wishbone", () => {
  assert.deepEqual(JV_RIG_SPACE_V1, {
    id: "jv-rig-space/v1", units: "metres", handedness: "right",
    forwardAxis: "+X", upAxis: "+Y", rightAxis: "+Z",
  ...
  assert.equal(mechanism.mechanismId, "m6.front-left.double-wishbone.legacy-procedural");
  assert.equal(mechanism.coordinateSpace, JV_RIG_SPACE_V1);
  assert.deepEqual(mechanism.bodies.map((value) => value.id), [
    "m6.chassis-reference", "m6.fl.upper-arm", "m6.fl.lower-arm", "m6.fl.carrier-reference",
  ]);
  assert.equal(mechanism.frames.length, 8);
  assert.deepEqual(mechanism.relations.map(({ id, type }) => ({ id, type })), [
...
test("legacy neutral projection is geometrically equivalent to the current FL suspension input", () => {
  ...
  const mechanism = projectLegacyM6FrontLeftWishboneNeutral(config);
  const upperHinge = midpoint(current.upperFrontChassis, current.upperRearChassis);
  const lowerHinge = midpoint(current.lowerFrontChassis, current.lowerRearChassis);

  closePoint(body(mechanism, "m6.fl.carrier-reference").neutralPose.position, rest, 1e-12, "carrier neutral origin");
  closePoint(body(mechanism, "m6.fl.upper-arm").neutralPose.position, upperHinge, 1e-12, "upper arm neutral origin");
  closePoint(body(mechanism, "m6.fl.lower-arm").neutralPose.position, lowerHinge, 1e-12, "lower arm neutral origin");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.upper-inboard.chassis"), upperHinge, 1e-12, "upper inboard chassis frame");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.upper-inboard.arm"), upperHinge, 1e-12, "upper inboard arm frame");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.lower-inboard.chassis"), lowerHinge, 1e-12, "lower inboard chassis frame");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.lower-inboard.arm"), lowerHinge, 1e-12, "lower inboard arm frame");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.upper-outboard.arm"), current.upperBallJoint, 1e-12, "upper outboard arm anchor");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.upper-outboard.carrier"), current.upperBallJoint, 1e-12, "upper outboard carrier anchor");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.lower-outboard.arm"), current.lowerBallJoint, 1e-12, "lower outboard arm anchor");
  closePoint(frameNeutralWorldPosition(mechanism, "m6.fl.lower-outboard.carrier"), current.lowerBallJoint, 1e-12, "lower outboard carrier anchor");

  for (const id of ["m6.fl.upper-inboard.chassis", "m6.fl.upper-inboard.arm", "m6.fl.lower-inboard.chassis", "m6.fl.lower-inboard.arm"]) {
  ...
  closePoint(current.upperBallJoint, { x: 1.2342520405653337, y: -0.37000000000000005, z: -0.8878987790374773 }, 1e-12, "accepted legacy upper ball");
  closePoint(current.lowerBallJoint, { x: 1.2657479594346663, y: -0.73, z: -0.9321012209625229 }, 1e-12, "accepted legacy lower ball");
});

test("neutral geometry receipt carries stand-alone source provenance without branch authority", () => {
  ...
  assert.deepEqual(receipt.source, TEST_SOURCE);
  assert.match(receipt.source.producer.commit, /^[0-9a-f]{40}$/);
  assert.match(receipt.source.configReceipt.gitBlob, /^[0-9a-f]{40}$/);
  assert.match(receipt.source.configReceipt.sha256, /^[0-9a-f]{64}$/);
...
test("neutral geometry receipt serialization is deterministic and round-trips exact logical content", () => {
  ...
  const second = serializeJvNeutralGeometryReceiptV1(buildLegacyM6FrontLeftNeutralGeometryReceipt(config, TEST_SOURCE));
